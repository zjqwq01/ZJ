...
class Scheduler(ABC):
    ...
    def discretize(self, time_steps):
        sigma_steps = self.get_sigma(time_steps[:-1])          # [100.00, 33.0349, 8.8564, 1.7480, 0.2106]
        sigma_steps = torch.cat([sigma_steps, torch.zeros_like(sigma_steps[:1])]) # [100.00,33.0349, 8.8564, 1.7480, 0.2106, 0]
...
class VPScheduler(Scheduler):
    ...
    def __init__(self, num_steps, beta_max=20, beta_min=0.1, epsilon=1e-5, beta_type='linear'):
        ...
        time_steps = self.get_discrete_time_steps(self.num_steps) # [1, 0.8, 0.6, 0.4, 0.2, 0.0]
