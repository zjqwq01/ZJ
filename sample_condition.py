...
def main():
    ...
    # MODEL_PATH = './models/lpips_alexnet.pth'
    # loss_fn_alex = lpips.LPIPS(net='alex', pretrained=False)
    # loss_fn_alex.load_state_dict(torch.load(MODEL_PATH))
    ...
    for i, ref_img in enumerate(loader):
        ...
        if measure_config['operator']['name'] in ['inpainting','inpainting_soc','super_resolution_inpainting_soc','super_resolution_inpainting']:
            ...
            sample_fn = partial(sample_fn, measurement_cond_fn=measurement_cond_fn, mask=mask, measure_config=measure_config)
        ...
        if 'soc' in measure_config['operator']['name'].split('_'):
            sample = sample_fn(x_start=x_start, measurement=y_n, operator = operator, record=True, save_root=out_path, solve_type=args.solve_type, gamma=args.gamma, time_travel=args.time_travel)
        else:
        ...
        print('TASK:{} Method:{} GAMMA:{} PSNR: {}, SSIM: {}, LPIPS: {}'.format(measure_config['operator']['name'], args.solve_type, args.gamma, psnr, ssim, lpips_alex)) # (18.9/0.61/0.222)(19.19/0.568/0.282)(19.22/0.701/0.249)
        ...
        if args.solve_type == 'linear':
            ...
            plt.imsave(os.path.join(out_path, 'recon_nonlinear', fname), clear_color(sample))
        elif args.solve_type == 'nonlinear_finitegamma':
            os.makedirs(os.path.join(os.path.join(out_path, 'recon_nonlinear_finitegamma'),str(args.gamma)), exist_ok=True)
            plt.imsave(os.path.join(os.path.join(out_path, 'recon_nonlinear_finitegamma'),str(args.gamma), fname), clear_color(sample))
...
# (30.46/0.869/0.05929)(30.08/0.8356/0.08566)(30.426/0.8744/0.09497)
